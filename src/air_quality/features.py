...
def build_dataset(raw_path: str, output_path: str, threshold: float = 100.0) -> pd.DataFrame:
    ...
    # Lag features — historical-only, no future data leakage
    df["pm2_5_lag_1"] = df["pm2_5"].shift(1)
    df["pm2_5_lag_3"] = df["pm2_5"].shift(3)
    # Rolling mean uses shift(1) so it never looks at the current row's value
    df["pm2_5_rolling_mean_6"] = df["pm2_5"].shift(1).rolling(6).mean()

    # Binary target: is next-hour PM2.5 above threshold?
...
if __name__ == "__main__":
    ...
    print(f"Target distribution:\n{frame['high_pollution_next_hour'].value_counts(normalize=True)}")
